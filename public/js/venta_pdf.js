(() => {
  ...
  function ensureModal() {
    const el = document.getElementById("modalPdfVenta");
    if (!el) {
      console.error("No existe #modalPdfVenta");
      return null;
    }
    ...
    if (!el.dataset.bound) {
      ...
      document.getElementById("btnPdfDescargar")?.addEventListener("click", () => {
        ...
        currentDoc.save(`venta-${Date.now()}.pdf`);
      });
  ...
  function buildPdf(data) {
    ...
    doc.text("ORDEN DE VENTA", PAGE_W / 2, 50, { align: "center" });
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(8.4);
    doc.text(`Cliente: ${clienteRaw}`, M, y);
...
rowPageBreak: "avoid"
    ...
    doc.text("GARANTÍA", M + 10, boxY + (noteH / 2) + 3);

    doc.setFont("helvetica", "normal");
    doc.setFontSize(7.3);

    const garantiaText = [
      "1. Garantía válida por 15 días desde la fecha de entrega (daños de fábrica).",
      "2. La empresa cubrirá la garantía del producto con la presente nota.",
      "3. No aplica garantía por uso inadecuado, malas instalaciones u otros factores."
    ];
    ...
    doc.setFontSize(8.2);
    doc.setTextColor(...DARK);
    doc.text(`Items: ${totalItems} • Venta total de: ${totalEnLetras}`, M, textY);
