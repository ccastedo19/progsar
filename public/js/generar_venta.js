(() => {
  ...
  function root() { return document.getElementById("generar-venta-page"); }
  function clienteSel() { return document.getElementById("ventaCliente"); }
  function porcentajeInput() { return document.getElementById("ventaPorcentajeFacturado"); }
  function deliveryInput() { return document.getElementById("ventaDelivery"); }
  function itemsList() { return document.getElementById("ventaItemsList"); }
  function productsGrid() { return document.getElementById("ventaProductsGrid"); }
  function subtotalBaseView() { return document.getElementById("ventaSubtotalBase"); }
  function deliveryView() { return document.getElementById("ventaDeliveryView"); }
  function pctView() { return document.getElementById("ventaPctView"); }
  function totalesWrap() { return document.getElementById("ventaTotalesWrap"); }
  function facturadoSwitch() { return document.getElementById("ventaFacturadoSwitch"); }
  function searchNombre() { return document.getElementById("ventaSearchNombre"); }
  function searchCodigo() { return document.getElementById("ventaSearchCodigo"); }
  function btnGuardar() { return document.getElementById("btnVentaGuardar"); }
  ...
  function showValidationErrors(errors) {
    const list = document.getElementById("ventaErroresList");
    if (!list) return;

    list.innerHTML = errors.map(err => `<li>${escapeHtml(err)}</li>`).join("");

    const modalEl = document.getElementById("modalVentaErrores");
  ...
  async function loadClientes() {
    try {
      ...
      const clientes = Array.isArray(data) ? data : (Array.isArray(data.data) ? data.data : []);
  ...
  async function loadProductos() {
    try {
      ...
      const productos = Array.isArray(data) ? data : (Array.isArray(data.data) ? data.data : []);
  ...
  function getAvailableStock(productId) {
    ...
    return Math.max(0, Number(product.stock || 0) - getReservedQty(productId));
  }
  ...
  function addProduct(productId) {
    ...
    if (availableStock <= 0) {
      showValidationErrors([`El producto "${product.nombre}" no tiene stock disponible.`]);
      return;
    }
    ...
    if (found) {
      ...
      state.items.push({
        ...
        precio_base: Number(product.precio_venta || 0),
  ...
  function increaseQty(productId) {
    ...
    if (availableStock <= 0) {
      showValidationErrors([`No puedes vender más del stock disponible para "${item.nombre}". Stock: ${item.stock}.`]);
      return;
    }
  ...
  function renderProducts() {
    ...
    productsGrid().innerHTML = list.map(p => `
      <div class="cot-product-card" data-add-product="${p.id}">
        <div class="cot-product-stock-badge">${getAvailableStock(p.id)}</div>
        ...
        <div class="cot-product-body">
          <div class="cot-product-head">
            ...
            <div class="cot-product-price">${money(p.precio_venta)}</div>
          </div>
        ...
        <div class="cot-item-content">
          <div class="cot-item-top">
            <div class="cot-item-info">
              ...
              <div class="cot-item-meta">
                Stock total: ${escapeHtml(item.stock)} | Disponible: ${getAvailableStock(item.id)}
              </div>
            </div>
          ...
          <div class="cot-item-edit-grid cot-item-edit-grid-compact">
            <div class="cot-field d-flex flex-row gap-2 align-items-center flex-wrap">
              <div>
                <label class="form-label form-label-sm mb-0">Precio Venta</label>
                <input
                  type="number"
                  step="0.01"
                  min="0.01"
  ...
  function validateVenta() {
    ...
    state.items.forEach((item, index) => {
      ...
      if (cantidad > Number(item.stock || 0)) {
        errors.push(`La cantidad del producto "${nombre}" supera el stock disponible (${item.stock}).`);
      }
    });
  ...
  function buildPdfData(payload, responseData) {
    return {
      numero: responseData?.numero || responseData?.venta_id || "S/N",
      fecha: payload.fecha,
  ...
  async function guardarVenta() {
    ...
    try {
      ...
      if (window.VentaPDF && typeof window.VentaPDF.open === "function") {
        ...
        console.error("VentaPDF no está cargado.", {
          ventaPdf: window.VentaPDF,
          jsPdf: window.jspdf,
          autoTable: window.jspdf?.jsPDF?.API?.autoTable
        });

        showValidationErrors([
          "La venta se registró correctamente, pero no se pudo abrir el PDF. Revisa la carga de /js/venta_pdf.js."
        ]);
      }
      ...
      console.error(error);
      showValidationErrors([error.message || "Ocurrió un error al registrar la venta."]);
    } finally {
      state.saving = false;

      if (btnGuardar()) {
        btnGuardar().disabled = false;
        btnGuardar().innerHTML = `<i class="bi bi-receipt-cutoff me-1"></i> Registrar Venta`;
      }
    }
  }
  ...
  window.initGenerarVentaPage = async function () {
    if (!root()) return;
    ...
    document.getElementById("btnVentaLimpiar").addEventListener("click", resetAll);
    document.getElementById("btnVentaGuardar").addEventListener("click", guardarVenta);
